...
client_to_use = 'Elasticsearch'

if client_to_use == 'Elasticsearch': 
    ...
    try:
        info = client.info()
        print("Connection to Elasticsearch succesful!")
...
def upload_post(client, scale):
    ...
    print(f"Post scale {scale}x Ready!!")
...
def upload_comments(client, scale):
    ...
    for chunk in tqdm(pd.read_csv(f'data_{scale}/Comments.csv', chunksize=1000)):
        chunk['UserId'] = chunk['UserId'].fillna(-1).astype(int)
...
def upload_commentjoinuser(client, scale):
    ...
    comments_join_users['UserId'] = comments_join_users['UserId'].fillna(-1).astype(int)
